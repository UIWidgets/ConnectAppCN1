...
namespace ConnectApp.components.pull_to_refresh
{
    ...
    public class _SmartRefresherState : State<SmartRefresher>
    {
        ...
        GlobalKey _headerKey = GlobalKey.key();
        GlobalKey _footerKey = GlobalKey.key();
        float _headerHeight = 0.0f;
        float _footerHeight = 0.0f;
        ValueNotifier<float> offsetLis = new ValueNotifier<float>(0.0f);
        ValueNotifier<int> topModeLis = new ValueNotifier<int>(0);
        ValueNotifier<int> bottomModeLis = new ValueNotifier<int>(0);
        ...
        bool _dispatchScrollEvent(ScrollNotification notification) {
            ...
            if (notification is ScrollUpdateNotification) {
                ...
                if (startNotification.dragDetails == null) {
                    ...
                    return _handleScrollMoving(startNotification);
                }
            }
