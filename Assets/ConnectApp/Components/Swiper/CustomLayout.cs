...
namespace ConnectApp.Components.Swiper {
    ...
    abstract class _CustomLayoutStateBase<T> : SingleTickerProviderStateMixin<T> where T : _SubSwiper {
        ...
        void _createAnimationController() {
            ...
            FloatTween tween = new FloatTween(begin: 0.0f, end: 1.0f);
            this._animation = tween.animate(this._animationController);
        }

        public override void didChangeDependencies() {
            WidgetsBinding.instance.addPostFrameCallback(this._getSize);
        ...
        Widget _buildAnimation(BuildContext context, Widget w) {
            ...
            return new GestureDetector(
                ...
                child: new ClipRect(
                    child: new Center(
                        child: this._buildContainer(list)
                    )
                )
            );
        }
        ...
        void _move(float position, int? nextIndex = null) {
            ...
            try {
                ...
                this._animationController.animateTo(position,
                    duration: TimeSpan.FromMilliseconds(this.widget.duration ?? 0),
                    curve: this.widget.curve);
                if (nextIndex != null) {
                    this.widget.onIndexChanged(this.widget.getCorrectIndex(nextIndex.Value));
                }
            }
            catch (Exception e) {
                Debug.LogError(e);
        ...
        void _onPanUpdate(DragUpdateDetails details) {
            ...
            this._animationController.setValue(value);
        }
    ...
    public class ScaleTransformBuilder : TransformBuilder<float> {
        ...
        public override Widget build(int i, float animationValue, Widget widget) {
            float s = CustomLayoutUtils._getValue(this.values, animationValue, i);
    ...
    public class OpacityTransformBuilder : TransformBuilder<float> {
        ...
        public override Widget build(int i, float animationValue, Widget widget) {
            float v = CustomLayoutUtils._getValue(this.values, animationValue, i);
            return new Opacity(
                opacity: v,
                child: widget
            );
    ...
    public class TranslateTransformBuilder : TransformBuilder<Offset> {
        ...
        public override Widget build(int i, float animationValue, Widget widget) {
            Offset s = CustomLayoutUtils._getOffsetValue(this.values, animationValue, i);
    ...
    public class CustomLayoutOption {
        public List<TransformBuilder> builders = new List<TransformBuilder> { };
