...
namespace ConnectApp.Api {
    public static class SearchApi {
        public static Promise<List<PopularSearch>> PopularSearch() {
            ...
            var request = HttpManager.GET("/api/search/popularSearch?searchType=project");
        ...
        public static Promise<FetchSearchResponse> SearchArticle(string keyword, int pageNumber) {
            ...
            var request = HttpManager.GET($"/api/search?t=project&projectType=article&k=[\"q:{keyword}\"]&searchAllLoadMore=false&page={pageNumber}");
            HttpManager.resume(request).Then(responseText => {
