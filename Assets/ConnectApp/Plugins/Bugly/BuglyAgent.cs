...
public sealed class BuglyAgent {
    ...
    public static void EnableExceptionHandler() {
        ...
        DebugLog(null,
            "Only enable the exception handler, please make sure you has initialized the sdk in the native code in associated Android or iOS project.");
    ...
    static readonly string GAME_AGENT_CLASS = "com.tencent.bugly.agent.GameAgent";
    static readonly int TYPE_U3D_CRASH = 4;
...
//                }
    ...
    static string _configChannel;
    static string _configVersion;
    static string _configUser;
    static long _configDelayTime;

    static void ConfigDefaultBeforeInit(string channel, string version, string user, long delay) {
    ...
    static bool _configCrashReporterPackage = false;

    static void ConfigCrashReporterPackage() {
    ...
    static void InitBuglyAgent(string appId) {
        ...
        try {
            GameAgent.CallStatic("initCrashReport", appId, _configChannel, _configVersion, _configUser,
                _configDelayTime);
            _isInitialized = true;
        }
        catch {
    ...
    static void ReportException(int type, string name, string reason, string stackTrace, bool quitProgram) {
        ConfigCrashReporterPackage();
    ...
    static void AddKeyAndValueInScene(string key, string value) {
        ConfigCrashReporterPackage();
    ...
    static void AddExtraDataWithException(string key, string value) {
        // no impl
    }

    static void LogRecord(LogSeverity level, string message) {
    ...
    static bool _crashReporterTypeConfiged = false;

    static void ConfigCrashReporterType() {
    ...
    static int LogSeverityToInt(LogSeverity logLevel) {
        int level;
        switch (logLevel) {
    ...
    static extern void _BuglyInit(string appId, bool debug, int level);

    [DllImport("__Internal")]
    static extern void _BuglySetUserId(string userId);

    [DllImport("__Internal")]
    static extern void _BuglySetTag(int tag);

    [DllImport("__Internal")]
    static extern void _BuglySetKeyValue(string key, string value);

    [DllImport("__Internal")]
    static extern void _BuglyReportException(int type, string name, string reason, string stackTrace, string extras,
        bool quit);

    [DllImport("__Internal")]
    static extern void _BuglyDefaultConfig(string channel, string version, string user, string deviceId);

    [DllImport("__Internal")]
    static extern void _BuglyLogMessage(int level, string tag, string log);

    [DllImport("__Internal")]
    static extern void _BuglyConfigCrashReporterType(int type);

    [DllImport("__Internal")]
    static extern void _BuglySetExtraConfig(string key, string value);
    ...
    static void ReportException(int type, string name, string message, string stackTrace, bool quitProgram) {
    }
    ...
    static event LogCallbackDelegate _LogCallbackEventHandler;

    static bool _isInitialized = false;
    static LogSeverity _autoReportLogLevel = LogSeverity.LogError;

    static int _crashReporterType = 1; // Default=0,1=Bugly-V2,MSDKBugly=2, IMSDKBugly=3
...
#pragma warning disable 414
    ...
    static bool _autoQuitApplicationAfterReport = false;

    static readonly int EXCEPTION_TYPE_UNCAUGHT = 1;
    static readonly int EXCEPTION_TYPE_CAUGHT = 2;
    static readonly string _pluginVersion = "1.5.1";

    static Func<Dictionary<string, string>> _LogCallbackExtrasHandler;
    ...
    static void _SetCrashReporterType(int type) {
    ...
    static void _SetCrashReporterLogLevel(int logLevel) {
#if UNITY_IOS
    ...
    static void _OnLogCallbackHandler(string condition, string stackTrace, LogType type) {
        _LogCallbackEventHandler?.Invoke(condition, stackTrace, type);
    ...
    static void _OnUncaughtExceptionHandler(object sender, UnhandledExceptionEventArgs args) {
    ...
    static void _HandleException(Exception e, string message, bool uncaught) {
        if (e == null) {
            return;
        }
        ...
        if (!string.IsNullOrEmpty(message)) {
            reason = $"{reason}{Environment.NewLine}***{message}";
        }
    ...
    static void _reportException(bool uncaught, string name, string reason, string stackTrace) {
            ...
            try {
                ...
                if (frames != null && frames.Length > 0) {
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        if (start > 0 && end > 0) {
                            trimFrameBuilder.AppendFormat("{0}(at {1}", frame.Substring(0, start).Replace(":", "."),
                                frame.Substring(end));
                        }
                        else {
        ...
        ReportException(uncaught ? EXCEPTION_TYPE_UNCAUGHT : EXCEPTION_TYPE_CAUGHT, name, reason, stackTrace,
            uncaught && _autoQuitApplicationAfterReport);
    }

    static void _HandleException(LogSeverity logLevel, string name, string message, string stackTrace, bool uncaught) {
        if (!IsInitialized) {
        ...
        if (!string.IsNullOrEmpty(message)) {
            try {
                if ((LogSeverity.LogException == logLevel) && message.Contains("Exception")) {
                    Match match =
                        new Regex(@"^(?<errorType>\S+):\s*(?<errorMessage>.*)", RegexOptions.Singleline).Match(message);
                    ...
                    Match match =
                        new Regex(@"^Unhandled\s+Exception:\s*(?<exceptionName>\S+):\s*(?<exceptionDetail>.*)",
                            RegexOptions.Singleline).Match(message);
                    ...
                    if (match.Success) {
                        ...
                        if (type.Equals("LuaScriptException") && exceptionDetail.Contains(".lua") &&
                            exceptionDetail.Contains("stack traceback:")) {
    ...
    static bool _uncaughtAutoReportOnce = false;
